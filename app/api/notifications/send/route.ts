    ...
    try {
      ...
      if (fcmToken) {
        ...
        try {
          ...
          // If token is invalid, remove it from all possible profile locations
          if (fcmError.code === 'messaging/registration-token-not-registered' || 
              fcmError.code === 'messaging/invalid-registration-token') {
            console.log(`[Notification] 🧹 Removing invalid FCM token for user ${userId}`);
            
            // Try to clear from users collection
            try {
            ...
            // Try to clear from worker_profiles
            try {
            ...
            // Try to clear from customer_profiles
            try {
